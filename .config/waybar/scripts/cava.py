...
class CavaServer:
    ...
    def _safe_remove_file(path):
        """Remove stale runtime files without crashing on permission issues"""
        try:
            os.remove(path)
    ...
    def _quick_check_running(self):
        ...
        if os.path.exists(self.socket_file):
            try:
                ...
                # The socket exists but is not accessible from this process.
                # Treat it as active and avoid unlinking a live socket we do not own.
                return True
            except (ConnectionRefusedError, FileNotFoundError, OSError, socket.timeout):
                self._safe_remove_file(self.socket_file)
        ...
        if os.path.exists(self.pid_file):
            try:
                ...
                try:
                    ...
                    self._safe_remove_file(self.pid_file)
...
class CavaClient:
    ...
    def __init__(self):
        ...
        self.last_tooltip = "Cava audio visualizer"

    def _get_media_tooltip(self):
        """Get current artist/title from playerctl for Waybar tooltip"""
        try:
            ...
            return self.last_tooltip

        tooltip = metadata.stdout.strip()
        ...
        try:
            ...
            if not (
                ...
                if json_output:
                    output = {
                        "text": standby_output,
                        "tooltip": self._get_media_tooltip(),
                    }
            ...
            print(
                "Error: Permission denied while connecting to cava manager socket",
                file=sys.stderr,
            )
            sys.exit(1)
